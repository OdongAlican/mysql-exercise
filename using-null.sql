...
SELECT teacher.name, COALESCE(teacher.mobile,'07986 444 2266') AS mobile_number FROM teacher
...
SELECT teacher.name AS name, COALESCE(dept.name, 'None')
FROM teacher LEFT JOIN dept
ON (teacher.dept=dept.id)
...
LEFT JOIN teacher ON (teacher.dept=dept.id) GROUP BY dept.name
