...
The ON says how to figure out which rows in game go with 
which rows in goal - the matchid from goal must match id from game. 
...
SELECT player, teamid, coach, gtime FROM eteam JOIN goal ON(eteam.id=goal.teamid) WHERE gtime<=10

/*
6.To JOIN game with eteam you could use either
game JOIN eteam ON (team1=eteam.id) or game JOIN eteam ON (team2=eteam.id)

Notice that because id is a column name in both 
game and eteam you must specify eteam.id instead of just id
...
WHERE ((team1='GER' OR team2='GER')) AND teamid != 'GER' GROUP BY player ORDER BY player
...
SELECT stadium, COUNT(player) AS total 
FROM game JOIN goal ON(game.id=goal.matchid) GROUP BY stadium
...
SELECT matchid, game.mdate, COUNT(player)
FROM game
JOIN goal
ON game.id = goal.matchid
...
team2,
SUM(CASE 
WHEN teamid=team2 THEN 1 ELSE 0 END)
score2
FROM game JOIN goal ON matchid = id
